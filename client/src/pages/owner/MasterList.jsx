...
import { PageHeader, Card, Input, Table, EmptyRow, RowActions } from '../../components/ui';
...
export default function MasterList() {
  ...
  // Cell display: booleans as Yes/No, refs show the stored id's name if present.
  const cell = (row, col) => {
  ...
  return (
    <div>
      <PageHeader eyebrow={`Master · ${def.menu}`} title={def.label}
        actions={<Link to={`/owner/${key}/create`} className="btn-primary">Add {def.label}</Link>} />

      <Card className="mb-4">
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={`Search ${def.label}…`} />
      </Card>
      ...
      <Table head={['ID', ...cols.map(labelFor), 'Actions']}>
        ...
        {!loading && rows.map((row) => (
          <tr key={row._id}>
            <td className="td font-mono text-xs text-ink-muted">{String(row._id).slice(-6)}</td>
            {cols.map((c) => <td key={c} className="td">{cell(row, c)}</td>)}
            <td className="td"><RowActions onEdit={() => nav(`/owner/${key}/${row._id}/edit`)} onDelete={() => del(row)} /></td>
          </tr>
        ))}
      </Table>
      <p className="mt-3 text-xs text-ink-muted">The ID is generated automatically by the database and isn't entered on the form.</p>
    </div>
  );
}
